...
# This is the map-frame pose of the spot where the reference scan is captured,
# and it is also the scan matcher's initial guess, so the car must physically be
# here when the GPS starts.
# The lab guide calibrates at node 11, whose true pose is [0, 2.163, -pi/2].
# Measured with pose_check.py on this car: declaring [0, 2, -pi/2] while
# calibrating at node 11 biased every reported position by 19 cm, which is 72%
# of a lane width. Corrected to node 11's actual pose.
