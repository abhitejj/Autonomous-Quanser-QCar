...
parser.add_argument('-a','--app', type=str, default="vehicle_control.py",
                    help="application to calibrate through. Must be the same "
                         "one that will drive, since its calibrationPose is "
                         "what labels the captured scan.")
...
# Calibrate through the driving application itself: the scan is labelled with
# whatever calibrationPose that file declares, so calibrating through a
# different app silently mislabels the frame.
cmd = f"cd {REMOTE_PATH} && python3 {APP} -c True"
stdin, stdout, stderr = ssh_cal.exec_command(cmd)
print(f"[{CALIBRATE_IP}] Running {APP} in calibration mode...")
